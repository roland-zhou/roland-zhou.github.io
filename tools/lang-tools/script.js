...
// Wrap everything in a closure to avoid pollution, but ensure global functions are accessible if needed
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init() {
        ...
        if (window.initSqlJs) {
            ...
            console.warn("SQL.js library not found (window.initSqlJs is undefined). Anki export on Android may fail.");
        }
    }
    ...
    if (ankiBtn) {
        ankiBtn.addEventListener('click', async (e) => {
            ...
            if (isAndroid) {
                ...
                if (!window.SQL) {
                    alert("Anki generator is still loading or failed to load. Please wait or refresh.");
                    return;
                }
                try {
    ...
    async function handleAction(action) {
        ...
        try {
            ...
            if (typeof constructPrompt === 'undefined' || typeof callGeminiAPI === 'undefined') {
                 throw new Error("Helper scripts not loaded.");
            }
